...
bins=(0:0.02:2)';                                  % está equiespaciada a 0.01, se crea un vector bins que definirá los bordes de cada pico en el eje frecuencial      
...
amp_fin=(amp_fin*t)/0.02;
x=(0.02:0.02:2)';
amp_fin = amp_fin(1:100);
figure, bar(x(1:100),amp_fin(1:100))
...
%%%%%%%%%%% cut-off de cada banda de frecuencia (VLF, LF y HF), para esto se
...
b4 = fittedmodel.b4; c4 = fittedmodel.c4;
...
m3 = 1/(c3^2) - 1/(c4^2);
n3 = b4/((c4^2)/2) - b3/((c3^2)/2);
p3 = (b3^2)/(c3^2) - (b4^2)/(c4^2) - log(c4/c3);

HF_cut_off = max(roots([m3 n3 p3]));

VHF_cut_off = b4 + 3*(c4/sqrt(2));    % HF cut-off según el paper se define como la media + 3*(desvío estándar)
Sdeviations = ([c1; c2; c3; c4])/sqrt(2);

Cut_offF = [VLF_cut_off;LF_cut_off;HF_cut_off;VHF_cut_off];
Medias = [b1; b2; b3; b4];
